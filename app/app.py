...
def load_data():
    ...
    # Pre-compute RGBA columns for pydeck
...
def render_tab_waste(df_full, kpi_tractor, kpi_global):
    ...
    view_mode = st.radio(
        "Visualization Mode:",
        ["💰 Waste Concentration (3D)", "🚜 Operational Trail (2D)"],
        horizontal=True,
    )

    if view_mode == "💰 Waste Concentration (3D)":
        st.caption("Columns represent **liters wasted**. Color = tractor identity.")
    else:
        st.caption("Points show the **path traveled**. Color = state (Green = Working, Red = Idle).")

    all_tractors = sorted(df_full['tractor'].unique())
    sel_tractors = st.sidebar.multiselect(
        "🚜 Tractors:", all_tractors, default=all_tractors, key="t1_tractors")
    ...
    step = st.sidebar.slider(
        "Sampling (skip every N points)", 1, 100, 95, key="t1_step",
        help="Increase if the map is slow. 1 = all points.")

    st.markdown("---")

    # Recalculate KPIs for selection
    ...
    df_map = df_full[df_full['tractor'].isin(sel_tractors)].iloc[::step].copy()
    ...
    if view_mode == "💰 Waste Concentration (3D)":
        ...
        if not df_idle.empty:
            ...
            for i, t in enumerate(unique_tractors):
                c = tractor_colors[t]
                with cols[i % 8]:
                    st.markdown(f"**<span style='color:rgb({c[0]},{c[1]},{c[2]})'>◼ {t}</span>**",
            ...
            layers.append(pdk.Layer(
                "ColumnLayer", df_agg,
                get_position=['lon_round', 'lat_round'], get_elevation='liters',
                get_fill_color='color', radius=5, elevation_scale=1000,
                pickable=True, auto_highlight=True, opacity=0.8))

            tooltip = {"html": "<b>{tractor}</b><br>Local waste: <b>{liters} L</b>",
                       "style": {"backgroundColor": "black", "color": "white"}}
            pitch = 60
        else:
            st.info("No idle data to display.")
...
def main():
    ...
    result = load_data()
    if result[0] is None:
        st.error("Data not found."); return
